...
def load_multi_model_file(folder_list: list):
    t1, dwi, t2wi, t2flair, swi, asl = folder_list
    ...
    for file in files:
        if file.endswith('T1WI.nii.gz'):
            ...
            t2wi_path = os.path.join(t2wi, file.replace('T1WI', 'T2WI'))
...
class MultiModelMRI(Dataset):
    ...
    def data_pre(self, index, aug_count=1, aug_index=(1,)):
        ...
        try:
            ...
            SWIimage = resize_image_itk(sitk.ReadImage(flnm['SWI']), newSpacing=spacing, newSize=newsize)
            ASLimage = resize_image_itk(sitk.ReadImage(flnm['ASL']), newSpacing=spacing, newSize=newsize)

        except Exception as e:
            print("error in loading DWI and bm")
            print(flnm)
            sys.exit(e)
        ...
        bm = sitk.GetArrayFromImage(SWIimage) > 0
        ...
        T2WI_std_image = standard_normalization(T2WIimage, remove_tail=False, divide='mean')[0] * 1000
        ...
        SWI_std_image, SWI_mean = standard_normalization(SWIimage, remove_tail=False, divide='mean')
        SWI_std_image = SWI_std_image * 1000
        SWI_std_image = sitk.Cast(SWI_std_image, sitk.sitkInt16)
        ...
        target_data_SWI = np.expand_dims(np.transpose(np.float32(sitk.GetArrayFromImage(SWI_std_image)) / 1000.0), 0)
        target_data_ASL = np.expand_dims(np.transpose(np.float32(sitk.GetArrayFromImage(ASL_std_image)) / 1000.0), 0)

        dataessamble['target'] = target_data_ASL

        try:
            dataessamble['Figure'] = np.concatenate(
              (input_data_T1, input_data_DWI, input_data_T2WI, input_data_T2FLAIR), 0)
        except Exception as e:
            print(flnm)
            sys.exit(e)

        dataessamble['root'] = flnm
        dataessamble['mean'] = ASL_mean
...
if __name__ == '__main__':
    folder_list = ['/home/Sdumt@us21201/data/UCSF-PDGM/T1WI',
                   '/home/Sdumt@us21201/data/UCSF-PDGM/DWI',
                   '/home/Sdumt@us21201/data/UCSF-PDGM/T2WI',
                   '/home/Sdumt@us21201/data/UCSF-PDGM/T2FLAIR',
                   '/home/Sdumt@us21201/data/UCSF-PDGM/SWI',
                   '/home/Sdumt@us21201/data/UCSF-PDGM/ASL'
                   ]
