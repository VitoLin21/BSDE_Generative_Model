...
class MultiModelMRI(Dataset):
    ...
    def data_pre(self, index, aug_count=1, aug_index=(1,)):
        ...
        total_space = [220, 220, 120]

        newsize = self.image_shape
        spacing = list(map(lambda x, y: x / y, total_space,  list(newsize)))
        ...
        # 配准T1图像
        if os.path.exists(flnm['T1'][:-4] + str(newsize) + 'r.nii.gz'):
            # 已配准
            T1_std_image = sitk.ReadImage(flnm['T1'][:-4] + str(newsize) + 'r.nii.gz')
        else:
            # 没配准的
            ...
            sitk.WriteImage(T1_std_image, flnm['T1'][:-4] + str(newsize) + 'r.nii.gz')

        # 配准ADC图像
        if os.path.exists(flnm['ADC'][:-4] + str(newsize) + 'r.nii.gz'):
            # 已配准
            ADC_std_image = sitk.ReadImage(flnm['ADC'][:-4] + str(newsize) + 'r.nii.gz')
        else:
            # 没配准的
            ...
            sitk.WriteImage(ADC_std_image, flnm['ADC'][:-4] + str(newsize) + 'r.nii.gz')
        ...
        sitk.WriteImage(CBV_std_image, flnm['CBV'][:-4] + str(newsize) + 'r.nii.gz')
        ...
        data = {
                'orig_size': dataessamble['affine']['size'],
                'aug_crops': aug_crops,
