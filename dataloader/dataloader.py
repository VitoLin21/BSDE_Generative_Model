...
def load_multi_model_file(folder_list: list):
    t1, t1c, adc, cbv, bm, t2wi, t2flair = folder_list
    ...
    for root, dirs, files in os.walk(t1):
        for file in files:
            if file.endswith('.nii'):
                ...
                bm_path = os.path.join(bm, head + 'T1C_bm_mask.nii.gz')
                t2wi_path = os.path.join(t2wi, head + 'T2WI.nii')
                t2flair_path = os.path.join(t2flair, head + 'T2FLAIR.nii')

                # 检查文件是否存在
                if all(os.path.exists(p) for p in
                       [t1_path, t1c_path, adc_path, cbv_path, bm_path, t2wi_path, t2flair_path]):
...
class MultiModelMRI(Dataset):
    ...
    def data_pre(self, index, aug_count=1, aug_index=(1,)):
        ...
        sitk.WriteImage(T1Cimage, flnm['T1C'][:-4] + str(newsize) + 'lr.nii.gz')
        ...
        # 配准T1图像
        if os.path.exists(flnm['T1'][:-4] + str(newsize) + 'lr.nii.gz'):
            # 已配准
            T1_std_image = sitk.ReadImage(flnm['T1'][:-4] + str(newsize) + 'lr.nii.gz')
        else:
            # 没配准的
            ...
            sitk.WriteImage(T1_std_image, flnm['T1'][:-4] + str(newsize) + 'lr.nii.gz')

        # 配准ADC图像
        if os.path.exists(flnm['ADC'][:-4] + str(newsize) + 'lr.nii.gz'):
            # 已配准
            ADC_std_image = sitk.ReadImage(flnm['ADC'][:-4] + str(newsize) + 'lr.nii.gz')
        else:
            # 没配准的
            ...
            sitk.WriteImage(ADC_std_image, flnm['ADC'][:-4] + str(newsize) + 'lr.nii.gz')

        # 配准CBV图像
        if os.path.exists(flnm['CBV'][:-4] + str(newsize) + 'lr.nii.gz'):
            # 已配准
            CBV_std_image = sitk.ReadImage(flnm['CBV'][:-4] + str(newsize) + 'lr.nii.gz')
        else:
            # 没配准的
            ...
            sitk.WriteImage(CBV_std_image, flnm['CBV'][:-4] + str(newsize) + 'lr.nii.gz')

        # 配准T2WI图像
        if os.path.exists(flnm['T2WI'][:-4] + str(newsize) + 'lr.nii.gz'):
            # 已配准
            T2WI_std_image = sitk.ReadImage(flnm['T2WI'][:-4] + str(newsize) + 'lr.nii.gz')
        else:
            # 没配准的
            T2WIimage = resize_image_itk(sitk.ReadImage(flnm['T2WI']), newSpacing=spacing, newSize=newsize)
            try:
                tx2 = register(T1Cimage, T2WIimage)
                resampler.SetTransform(tx2)
            except:
                print(flnm['T2WI'])
                tx1 = sitk.CenteredTransformInitializer(sitk.Cast(brainmask, T2WIimage.GetPixelID()),
                                                        sitk.Cast(T2WIimage, T2WIimage.GetPixelID()),
                                                        sitk.Euler3DTransform(),
                                                        operationMode=sitk.CenteredTransformInitializerFilter.GEOMETRY)
                resampler.SetTransform(tx1)
            resampler.SetReferenceImage(brainmask)
            rsz_image = resampler.Execute(T2WIimage)

            rsz_image = sitk.Cast(rsz_image, sitk.sitkInt16) * sitk.Cast(brainmask, sitk.sitkInt16)
            std_image = standard_normalization(rsz_image, remove_tail=False, divide='mean') * 1000
            T2WI_std_image = sitk.Cast(std_image, sitk.sitkInt16)
            sitk.WriteImage(T2WI_std_image, flnm['T2WI'][:-4] + str(newsize) + 'lr.nii.gz')

        # 配准T2FLAIR图像
        if os.path.exists(flnm['T2FLAIR'][:-4] + str(newsize) + 'lr.nii.gz'):
            # 已配准
            T2FLAIR_std_image = sitk.ReadImage(flnm['T2FLAIR'][:-4] + str(newsize) + 'lr.nii.gz')
        else:
            # 没配准的
            T2FLAIRimage = resize_image_itk(sitk.ReadImage(flnm['T2FLAIR']), newSpacing=spacing, newSize=newsize)
            try:
                tx2 = register(T1Cimage, T2FLAIRimage)
                resampler.SetTransform(tx2)
            except:
                print(flnm['T2FLAIR'])
                tx1 = sitk.CenteredTransformInitializer(sitk.Cast(brainmask, T2FLAIRimage.GetPixelID()),
                                                        sitk.Cast(T2FLAIRimage, T2FLAIRimage.GetPixelID()),
                                                        sitk.Euler3DTransform(),
                                                        operationMode=sitk.CenteredTransformInitializerFilter.GEOMETRY)
                resampler.SetTransform(tx1)
            resampler.SetReferenceImage(brainmask)
            rsz_image = resampler.Execute(T2FLAIRimage)

            rsz_image = sitk.Cast(rsz_image, sitk.sitkInt16) * sitk.Cast(brainmask, sitk.sitkInt16)
            std_image = standard_normalization(rsz_image, remove_tail=False, divide='mean') * 1000
            T2FLAIR_std_image = sitk.Cast(std_image, sitk.sitkInt16)
            sitk.WriteImage(T2FLAIR_std_image, flnm['T2FLAIR'][:-4] + str(newsize) + 'lr.nii.gz')
        ...
        input_data_T2WI = np.expand_dims(np.transpose(np.float32(sitk.GetArrayFromImage(T2WI_std_image)) / 1000.0), 0)
...
if __name__ == '__main__':
    folder_list = ['/home/Sdumt@us21101/data/BSDE_T1WI',
                   ...
                   '/home/Sdumt@us21101/data/brainmask_1230',
                   '/home/Sdumt@us21101/data/BSDE_T2WI',
                   '/home/Sdumt@us21101/data/BSDE_T2FLAIR'
                   ]
