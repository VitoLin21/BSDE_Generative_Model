...
class MultiModelMRI(Dataset):
    ...
    def data_pre(self, index, aug_count=1, aug_index=(1,)):
        ...
        # dataessamble['Figure'] = np.concatenate(
        #     (input_data_T1, input_data_T1C, input_data_T2WI, input_data_T2FLAIR), 0)
...
if __name__ == '__main__':
    ...
    dataset = MultiModelMRI(total, image_shape=(256, 256, 32), transform=transform)
    dataloader = DataLoader(dataset, batch_size=20, shuffle=True, num_workers=4)
    for batch in dataloader:
        # 这里可以加入你的训练或验证逻辑
        print(batch['Figure'].mean(), batch['target'].mean())