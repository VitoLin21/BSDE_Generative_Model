...
from dataloader.infer_dataloader_3modal_cbv import MultiModelMRI, ToTensor, load_multi_model_file

from sklearn.model_selection import train_test_split
from skimage.metrics import structural_similarity as ssim, peak_signal_noise_ratio

logdir = 'log/3modal_new_cbv'
...
def test(model, dataloader, device):
    ...
    with torch.no_grad():
        for idx, data in enumerate(dataloader):
            ...
            ssim, mae, mse, psnr, ncc = calculate_ssim(target[0][0], y[0][0], mean)
            total_ssim += ssim
            total_mae += mae
            total_mse += mse
            total_psnr += psnr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dim_x', default=128, type=int)
    parser.add_argument('--dim_y', default=128, type=int)
    ...
    train(model, multi_dataloader, test_dataloader, optimizer, n_epochs, store_path, device, local_rank, best_test_path)
