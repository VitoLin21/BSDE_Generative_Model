...
class Z_nn(nn.Module):
    def __init__(self, norm_name="instance", num_of_res=6, modal=3):
        """
        Args:
            norm_name:归一化方法
            num_of_res: 残差块的数量
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv3d(modal, 8, kernel_size=7, stride=1),
            nn.InstanceNorm3d(8),
            nn.ReLU()
        )
...
class FBSDEGen(nn.Module):
    def __init__(self, dim_x, dim_y, dim_h1=1000, dim_h2=600, dim_h3=1000, T=1.0, N=2, device=None, modal=5, image_shape=(128, 128, 64)):
        ...
        # self.z_nn = Z_nn(norm_name="instance", num_of_res=6, modal=self.modal)
        # self.y0_nn = Z_nn(norm_name="instance", num_of_res=6, modal=self.modal)

        self.z_nn = SwinUNETR(img_size=image_shape, in_channels=self.modal, out_channels=1, use_v2=True)
        self.y0_nn = SwinUNETR(img_size=image_shape, in_channels=self.modal, out_channels=1, use_v2=True)

        # self.z_nn = UNETR(img_size=(128, 128, 64), in_channels=self.modal, out_channels=1)
        # self.y0_nn = UNETR(img_size=(128, 128, 64), in_channels=self.modal, out_channels=1)
    ...
    def forward(self, input):
        ...
        for i in range(self.N):
            dw = torch.randn(z.size()).to(self.device) * 0.001
            y = (y - self.f(y, z) * 0.5 +
...
if __name__ == '__main__':
    ...
    x = torch.randn(2, 4, 256, 256, 32).to(device)
